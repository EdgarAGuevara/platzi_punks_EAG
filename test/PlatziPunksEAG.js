...
describe('PLazi Punks Contract', () => {

    const setup = async ({ maxSupply = 10000 }) => {
        const [owner] = await ethers.getSigners();
        const PlatziPunksEAG = await ethers.getContractFactory("PlatziPunksEAG");
        const deployed = await PlatziPunksEAG.deploy(maxSupply);
    ...
    describe('Minting', () => {
        ...
        it('Has a minting limit', async () => {
            ...
            await expect(deployed.mint()).to.be.revertedWith("No PlatziPunks EAG left :(");
        })
    });
